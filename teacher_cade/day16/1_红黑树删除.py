...
class RedBlackTree(object):
    ...
    def insert(self, node):
        ...
        # 在相应位置插入节点
        if not temp_node:  # 放入根
            self.root = node
            node.color = BLACK
        elif node.value < temp_node.value:  # 放在父亲的左边
            temp_node.left = node
            node.p = temp_node
        else:  # 放在父亲的右边
...
def main():
    ...
    for number in number_list:
        node = RedBlackTreeNode(number)  # 为一个节点申请空间
        tree.insert(node)
    ...
    # 删除
    print('-' * 50)
    del_node = tree.find_node(6)
    tree.delete(del_node)
    del_node = tree.find_node(9)
    tree.delete(del_node)
    del_node = tree.find_node(8)
    tree.delete(del_node)
    # preorder_tree_walk(tree.root)
    rbtree_print(tree.root, tree.root.value, 0)
